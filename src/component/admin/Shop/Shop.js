				...
				render: (text, record) => {
					return (
						<Tooltip placement="top" title={record.address}>
							<span className="common_table_ellipse">
								{record.address}
							</span>
						</Tooltip>
					);
				...
				render: (text, record) => {
					return (
						<span className="common_table_span">
							<a
								href="javascript:;"
								onClick={this.onEditorCampus.bind(this, record)}
							>
								修改
							</a>
							<a
								href="javascript:;"
								onClick={this.controllerMapDialogVisible.bind(
									this,
									record
								)}
							>
							...
							<Popconfirm
								placement="top"
								title="是否确认删除"
								onConfirm={this.onConfirmDelete.bind(
									this,
									record
								)}
								okText="确认"
							...
							<a
								href="javascript:;"
								onClick={this.modifyAccount.bind(this, record)}
							>
								修改账户
							</a>
							<a
								href="javascript:;"
								onClick={this.getMore.bind(this, record)}
							>
								更多
							</a>
						</span>
			...
			<div className="common">
				<div className="common_search">
					<Form className="common_search_form" {...formItemLayout}>
						...
						<Col span={6} offset={1}>
							...
							<Button
								type="primary"
								onClick={this.controllerAddDialog.bind(this)}
							>
								新增
							</Button>
						</Col>
				...
				{addDialogVisible && (
					<AddDialog
						...
						controllerAddDialog={this.controllerAddDialog.bind(
							this
						)}
					/>
				)}
				{accountVisible && (
					<UserDialog
						shopid={shopid}
						onSearch={this.onSearch.bind(this)}
						onControllerAccountDialog={this.onControllerAccountDialog.bind(
							this
						)}
					/>
				)}
				{editorDialogVisible && (
					<EditorDialog
						editData={editData}
						onSearch={this.onSearch.bind(this)}
						controllerEditorDialog={this.controllerEditorDialog.bind(
							this
						)}
					/>
				)}
				{dataDialogVisible && (
					<DataDialog
						shopid={shopid}
						onControllerDataVisible={this.onControllerDataVisible.bind(
							this
						)}
					/>
				)}
				{addressDialogVisible && (
					<MapDialog
						shopid={shopid}
						onControllerMapDialog={this.controllerMapDialogVisible.bind(
							this
						)}
					/>
				)}
