		...
		const columns = [
			...
			},
			{
				...
				render: (text, record) => {
					...
					if (status == 1)
						return <span style={{ color: '#15863d' }}>{Filter.filterBillStatus(record.status)}</span>;
					if (status == 2)
						return <span style={{ color: 'red' }}>{Filter.filterBillStatus(record.status)}</span>;
					if (status == 3)
						return <span style={{ color: '#008dff' }}>{Filter.filterBillStatus(record.status)}</span>;
					if (status == 4)
						return <span style={{ color: 'red' }}>{Filter.filterBillStatus(record.status)}</span>;
				},
			},
						...
						return (
							<span className="common_table_span">
								<Popconfirm
									placement="top"
									title="是否确认拒绝"
									onConfirm={this.onChangeBillStatus.bind(this, record, 2)}
									okText="确认"
									cancelText="取消"
								>
								...
								<Popconfirm
									placement="top"
									title="是否确认完成支付"
									onConfirm={this.onChangeBillStatus.bind(this, record, 3)}
									okText="确认"
									cancelText="取消"
								>
